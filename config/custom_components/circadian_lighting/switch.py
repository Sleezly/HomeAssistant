...
CONF_LIGHTS_CT_NOBRIGHTNESS = 'lights_ct_nobrightness'
...
PLATFORM_SCHEMA = vol.Schema({
    ...
    vol.Optional(CONF_LIGHTS_CT_NOBRIGHTNESS): cv.entity_ids,
    ...
    vol.Optional(CONF_LUX_ENTITY): cv.entity_id,
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    if cl:
        ...
        lights_ct_nobrightness = config.get(CONF_LIGHTS_CT_NOBRIGHTNESS)
        ...
        lux_entity = config.get(CONF_LUX_ENTITY)
        initial_transition = config.get(CONF_INITIAL_TRANSITION)
        cs = CircadianSwitch(hass, cl, name, lights_ct, lights_ct_nobrightness, lights_rgb, lights_xy, lights_brightness,
                                disable_brightness_adjust, min_brightness, max_brightness,
                                sleep_entity, sleep_state, sleep_colortemp, sleep_brightness,
                                disable_entity, disable_state, lux_entity, initial_transition)
        add_devices([cs])
...
class CircadianSwitch(SwitchDevice, RestoreEntity):
    ...
    def __init__(self, hass, cl, name, lights_ct, lights_ct_nobrightness, lights_rgb, lights_xy, lights_brightness,
                    disable_brightness_adjust, min_brightness, max_brightness,
                    sleep_entity, sleep_state, sleep_colortemp, sleep_brightness,
                    disable_entity, disable_state, lux_entity, initial_transition):
        ...
        self._lights_ct_nobrightness = lights_ct_nobrightness
        ...
        if lights_ct_nobrightness != None:
            self._lights += lights_ct_nobrightness
        if lights_rgb != None:
    ...
    def calc_brightness(self):
        if self._disable_brightness_adjust is True:
            ...
            elif self._lux_entity is not None:
                _LOGGER.debug(self._name + " in Lux mode " + str(((self._min_brightness + (self._min_brightness * self._max_brightness)) / 100) * (1.0 - max(0, int(self.hass.states.get(self._lux_entity).state) - 15000 / max(35000 - 15000, int(self.hass.states.get(self._lux_entity).state)) - 15000))))
    ...
    def adjust_lights(self, lights, transition=None):
        if self.should_adjust():
            ...
            for light in lights:
                ...
                """Set color of array of ct light no brightness if on."""
                if self._lights_ct_nobrightness is not None and light in self._lights_ct_nobrightness and is_on(self.hass, light):
                    service_data = {ATTR_ENTITY_ID: light}
                    if mired is not None:
                        service_data[ATTR_COLOR_TEMP] = mired
                    if transition is not None:
                    ...
                    self.hass.services.call(
                        LIGHT_DOMAIN, SERVICE_TURN_ON, service_data)
                    _LOGGER.debug(light + " CT No Brightness Adjusted - color_temp: " + str(mired) + ", transition: " + str(transition))
